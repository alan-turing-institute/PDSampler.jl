...
@test maximum(abs.(loglik.(mvg, xvals) - check_loglik.(xvals))) <= 1e-6

# canon to natural
@test maximum(abs.(loglik.(mvg, xvals) - loglik.(mvg2, xvals))) <= 1e-10

# standard to diagonal
@test maximum(abs.(loglik.(mvg_da, xvals) - loglik.(mvg_db, xvals))) <= 1e-10
