...

{
    ...
    "category": "section",
    "text": "(the code for this example can be found here, note that the doc rendered here was automatically generated, if you want to fix it, please do it in the julia code directly)In this example we use the global Bouncy Particle Sampler on 2D Gaussian truncated to the positive orthan to show how to declare a BPS model.(Image: )Start by loading the library:julia using PDMP you then need to define two elements:1. a geometry (boundaries)2. an energy (gradient of the log-likelihood of the target)The positive orthan corresponds to a simple Polygonal domain where the boundaries are the axes. The normal to these boundaries (ns) are therefore unit vectors and the intercepts (a) are zero.A polygonal domain is then declared with the constructor Polygonal.julia p = 2# normal to faces and interceptsns, a = eye(p), zeros(p)geom  = Polygonal(ns, a) The function nextboundary returns a function that can compute the next boundary on the current ray [x,x+tv] with t>0 as well as the time of the hit.julia nextbd(x, v) = nextboundary(geom, x, v) The model then needs to be specified: you need to define a function of the formgradll(x) which can return the gradient of the log-likelihood at some point x.Here, let us consider a 2D gaussian.```julia"
},

{
    "location": "examples/ex_gbps1.html#here-we-build-a-valid-precision-matrix.-The-cholesky-decomposition-of#-the-covariance-matrix-will-be-useful-later-to-build-a-sensible#-starting-point-for-the-algorithm.srand(12)P1-randn(p,p)P1-*-P1'P1-norm(P1)/100*eye(p)C1-inv(P1);-C1-C1';-C1/2;L1-cholfact(C1)mu-zeros(p)1.mvg-MvGaussianCanon(mu,-P1)-1",
    "page": "Global BPS",
    "title": "here we build a valid precision matrix. The cholesky decomposition of# the covariance matrix will be useful later to build a sensible# starting point for the algorithm.srand(12)P1  = randn(p,p)P1 = P1'P1 += norm(P1)/100eye(p)C1  = inv(P1); C1 += C1'; C1/=2;L1  = cholfact(C1)mu  = zeros(p)+1.mvg = MvGaussianCanon(mu, P1)```",
    "category": "section",
    "text": "Here, we have defined the gaussian through the Canonical representation i.e.: by specifying a mean and a precision matrix.Every model must implement a gradloglik function returning the gradient of the log-likelihood at a point x.julia gradll(x) = gradloglik(mvg, x) Next, you need to define the function which can return the first arrival time ofthe corresponding Inhomogenous Poisson Process.Note that you could be using nextevent_zz here as well if you wanted to usethe Zig-Zag sampler (and you could implement other kernels as well).julia nextev(x, v) = nextevent_bps(mvg, x, v) For a Gaussian (and some other simple distributions), this is analytical throughan inversion-like method.Finally, you need to specify the parameters of the simulation such as thestarting point and velocity, the length of the path generated, the rate ofrefreshment and the maximum number of gradient evaluations.julia T    = 1000.0   # length of path generatedlref = 2.0      # rate of refreshmentx0   = mu+L1[:L]*randn(p) # sensible starting pointv0   = randn(p) # starting velocityv0  /= norm(v0) # put it on the sphere (not necessary)# Define a simulationsim = Simulation( x0, v0, T, nextev, gradll,                  nextbd, lref ; maxgradeval = 10000) And finally, generate the path and recover some details about the simulation.julia (path, details) = simulate(sim) The path object belongs to the type Path and can be sampled usingsamplepath.A crude sanity check is for example to check that the estimated mean obtained through quadrature along the path yields a similar result as a basic Monte Carlo estimator.```julia"
},

{
    "location": "examples/ex_gbps1.html#Building-a-basic-MC-estimator#-(taking-samples-from-2D-MVG-that-are-in-positive-orthan)sN-1000s-broadcast(,-mu,-L1[:L]*randn(p,sN))mt-zeros(2)np-0#-Sum-for-all-samples-in-the-positive-orthanss-[s;-ones(sN)']mt-sum(ss[:,i]-for-i-in-1:sN-if-!any(e-e0,-ss[1:p,i]))mt-mt[1:p]/mt[end]-1",
    "page": "Global BPS",
    "title": "Building a basic MC estimator# (taking samples from 2D MVG that are in positive orthan)sN = 1000s  = broadcast(+, mu, L1[:L]*randn(p,sN))mt = zeros(2)np = 0# Sum for all samples in the positive orthanss = [s; ones(sN)']mt = sum(ss[:,i] for i in 1:sN if !any(e->e<0, ss[1:p,i]))mt = mt[1:p]/mt[end]```",
    "category": "section",
    "text": "You can now compare the norm of mt (a crude MC estimator) to pathmean(path) (computing the integrals along the segments of the path) and you will see thatthe relative error is below 5%."
},
...

{
    ...
    "category": "section",
    "text": "(the code for this example can be found here, note that the doc rendered here was automatically generated, if you want to fix it, please do it in the julia code directly)The approach to using the local BPS is much the same as for the global oneexcept that you need to specify a FactorGraph.That object will contain the structure of the factor graph (i.e.: which factor is connected to which variables) as well as the list of all factors (which have a lgradll and nextevent since each factor can be seen individually as a small BPS).Below, we show how to declare a chain of bivariate gaussians:julia using PDMPnfac = 3 # number of factorsmvg = MvGaussianStandard(zeros(2),eye(2))# all factors have that same likelihoodchainfactor(i) = Factor(                    (x,v)->nextevent_bps(mvg, x, v),                    (x)->gradloglik(mvg, x),                    i )# assemble into a chain graphchain = chaingraph([chainfactor(i) for i in 1:nfac]) This is a simple graph with a known structure so that it's already definedthrough the chaingraph function (in src/local/factorgraph.jl). For anarbitrary graph, you would need to provide two things:1. the structure of the factor graph: a list of list where each element corresponds to a factor and the corresponding list contains the indices of the variables attached to that factor2. the list of factorsThe rest is very similar to the global BPS:julia srand(123)lambdaref  = .01maxnevents = 10000T          = Infnvars      = chain.structure.nvarsx0         = randn(nvars)v0         = randn(nvars)v0        /= norm(v0)lsim = LocalSimulation(chain, x0, v0, T, maxnevents, lambdaref)(all_evlist, details) = simulate(lsim) The all_evlist object contains a list of EventList corresponding to whathappened on each of the factors. It can also be sampled using samplelocalpath(cf. src/local/event.jl)."
},
